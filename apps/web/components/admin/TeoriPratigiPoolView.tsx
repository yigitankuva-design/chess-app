...
interface Props {
  pool: TeoriPratigiQuestion[];
  onAddQuestion: (q: TeoriPratigiQuestion) => Promise<void>;
  onUpdateQuestion: (id: string, next: TeoriPratigiQuestion) => Promise<void>;
...
/**
 * b) Teori Pratiği'nin soru havuzu — KonumPratigiPoolView.tsx ile AYNI
 * desen (Kazanç Konumu'ndaki Konum Havuzu, bkz. PositionPoolView.tsx):
 * kapalı/açık "Konum Havuzu" kartı, kodlar yuvarlak düğmeler halinde 12'li
 * satırlarda, tıklayınca düzenleme (TeoriPratigiFields initial-dolu), Sil
 * düzenleme başlığının yanında.
 */
export function TeoriPratigiPoolView({ pool, onAddQuestion, onUpdateQuestion, onDeleteQuestion }: Props) {
  ...
  return (
    <div className="space-y-3">
      <TeoriPratigiFields onSubmit={onAddQuestion} />
                ...
                <div key={`edit-${p.id}`} className="space-y-3 pt-2 border-t border-white/10">
                  <div className="flex items-center justify-between">
                    ...
                    <button type="button" onClick={() => { onDeleteQuestion(p.id); setDuzenlenen(null); }}
                      aria-label={`${kod} kodlu Teori Pratiği sorusunu sil`}
