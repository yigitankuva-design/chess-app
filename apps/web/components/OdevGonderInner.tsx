 ...
 * "Ödev Gönder"e basınca gelinen ekran. Akış (Zafer'in görseli):
 ...
 *
 * Madde 2026-09-16 (Antrenör Ekranı, Faz C): `apps/web/app/(teacher)/coach/
 * odev-gonder/page.tsx`'ten `components/` altına taşındı — bir `page.tsx`
 * `default` dışında bileşen export EDEMEZ (bkz. tests/page-exports-guard.
 * test.ts, "turnuva sayfaları günlerce canlıya çıkamadı" dersi). Route
 * sayfası `sectionId`'yi query'den okuyup buraya PROP olarak iletir; canlı
 * ders ekranı da (route DEĞİŞTİRMEDEN, LiveKitRoom'u unmount etmeden) AYNI
 * bileşeni gömülü bir panelde kullanır (bkz. dersler-canli/[id]/page.tsx).
 */
...
interface Props {
  sectionId: number;
  /** Verilirse "Geri dön"/başarı sonrası dönüş `router.back()`/
   *  `router.push('/coach')` YERİNE bu çağrılır — canlı ders içinde gömülü
   *  kullanımda bir modal/panel kapatma işlevi görür. Verilmezse (route
   *  kullanımı) davranış AYNEN korunur. */
  onClose?: () => void;
}

export function OdevGonderInner({ sectionId, onClose }: Props) {
  const router = useRouter();
  const { role, token } = useAuth();
  const goBack = onClose ?? (() => router.back());
  const goToCoachHome = onClose ?? (() => router.push('/coach'));
  ...
  if (done != null) {
    return (
      <main className="px-4 pt-6 pb-12 max-w-xl mx-auto space-y-4">
        ...
        <button type="button" onClick={goToCoachHome}
